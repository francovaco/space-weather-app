...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await instrumentedFetch(url, { signal: controller.signal, cache: 'no-store', headers: { 'User-Agent': 'space-weather-app/0.1', 'Accept-Encoding': 'identity' } }, 'swpc/proton-flux')
    ...
    logger.error('Failed to fetch proton flux data', { route: 'swpc/proton-flux', url, err })
